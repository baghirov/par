...
class Constants(BaseConstants):
    ...
    players_per_group = 2
    num_rounds = 5
    instructions_template = 'part4/Instructions.html'

    endowment = c(0)
    bad_payment_1 = c(-20)
    good_payment_1 = c(10)
    
    # in treatment 4, stakes increase by multiplying with 4. Thus, multiply
    # good and bad payment with 4 in the second period. With 16 in the third 
    ...
    # in quiz, each question worths 2 Tl. exchange rate is 1/21 hence 42 points:
    point_per_question = c (42) 
    outside_payment = c(0)
...
class Subsession(BaseSubsession):
    ...
    def creating_session(self):
        ...
        #set types for each subject individually
        ## rdm = random.uniform(0,1)
       
        ##for g in self.get_groups():
        ##    agent = g.get_player_by_role('agent')
        ##    if self.round_number == 1:
        ##        if rdm < Constants.probability_unbiased:
        ##            agent.type = 0
        ...
        #set types for as percentage f the population. of 10 agents, 1 is good:
        for g in self.get_groups():
            agent = g.get_player_by_role('agent')
            if self.round_number == 1:
...
class Group(BaseGroup):
    ...
    def set_return_2(self):
        ...
        if self.agent_hired_2:
            principal.pyf_2 = Constants.multiplier * unbiased_return(self.agent_action_2, self.state_of_world_2)
            if self.agent_type == 0:
                agent.pyf_2 = Constants.multiplier  * unbiased_return(self.agent_action_2, self.state_of_world_2)
            else: 
                agent.pyf_2 = Constants.multiplier  * biased_return(self.agent_action_2)   
        else:
            principal.pyf_2 = c(0)
            agent.pyf_2 = c(0)
    ...
    def set_return_3(self):
        ...
        if self.agent_hired_3:
            principal.pyf_3 =  Constants.multiplier*Constants.multiplier* unbiased_return(self.agent_action_3, self.state_of_world_3)
            
            if self.agent_type == 0:
                agent.pyf_3 =  Constants.multiplier*Constants.multiplier* unbiased_return(self.agent_action_3, self.state_of_world_3)
            else: 
                agent.pyf_3 =  Constants.multiplier*Constants.multiplier* biased_return(self.agent_action_3)   
    ...
    def set_round_payoffs(self):
        ...
        for p in players:
            ...
            print ("player round payoff", p.pyf)
